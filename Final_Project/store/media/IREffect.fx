...
// This is the reciprocal of the maximum depth range. It is currently set to 1 / 0.8 meters.
...
#define invDepthRange 1.25

// The half angle of the IR sensor cone, 1 degrees
#define SensorConeHalfAngleRadians 0.017453

// Minimum and Maximum range of the sensor
#define MinimumRangeMM 100.0
#define MaximumRangeMM 800.0

// How transparent can a polygon be and still produce a depth value. 50%
#define TransparentThreshold 0.5
              ...
              ) : COLOR
{
    ...
    if (useTexture)
    {
        if (useWrappedTexture)
        {
            texColor = tex2D(g_samScene, Tex);
        }
        else
        {
            texColor = tex2D(g_samSceneNoWrap, Tex);
        }
    }
    else
    {
    ...
    {
        if (texColor.a > TransparentThreshold)
        {
        ...
        {
            // Make it fully translucent so we pick up objects behind the glass
            outColor.a = 0.0;
        }
    }
    else
    {
        // Culled due to cone angle
        outColor.rgb = 1.0;
    }
    return outColor;
}
