    ...
    for( int LightIndex = 0; LightIndex < ACTIVE_LIGHT_COUNT; ++LightIndex )
    {
        if (CanUseLight(renderMode, LightIndex))
        {
            ...
            if (LIGHT_TYPE[LightIndex] == LIGHTTYPE_POINT)
            {
				...
				intensity = lightIntensityVector[LightIndex]; 
            }
            ...
            else
            {
                ...
                intensity = step(spotCosfloatUmbra[LightIndex], rho);
            }
            diffuse += saturate(dot(lightdir, nNormal)) * intensity; 
        }
    } 

    diffuse *= matDiffuse;
    float4 texelColor = texColor*(ambient + diffuse);
    
    // Apply fog
    if (renderMode != ONLY_USE_SHADOW_CASTER_LIGHT)
    {
        float fogScale = length(FogVec) - FOG_START/(FOG_END-FOG_START);
        texelColor = lerp(texelColor, fogColor, saturate(fogScale));
    }
    else
    {
