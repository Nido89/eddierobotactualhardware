...
float    spotCosHalfUmbra[MAX_NUM_LIGHTS]; // cos(theta/2) for spotlights
...
// This is the reciprocal of the maximum depth range. It is currently set to 1 / 2.0 meters.
// This constant must be updated if the maximum depth range of the depth camera is changed.
// not ideal, but no way to set fx parameters from sim through standard routes
#define invDepthRange 0.5

// The half angle of the sonar sensor cone, 22.5 degrees
#define SensorConeHalfAngleRadians 0.39269

// Minimum and Maximum range of the sensor
#define MinimumRangeMM 100.0
#define MaximumRangeMM 2000.0

// The angle of incidence required to produce a depth value, 45 degrees
#define AngleOfIncidenceThresholdRadians 0.78539
#define PI 3.141593
              ...
              ) : COLOR
{
    float4 outColor;
    outColor.a = 1.0;
    float3 normalizedPosVec = normalize(mul(eyePos-Pos, viewMatrix));
    float3 normalizedEyeVec = float3(0.0, 0.0, 1.0);
    float3 normalizedNormal = normalize(mul(Normal,viewMatrix));
    float angleToConeRadians = abs(acos(dot(normalizedPosVec, normalizedEyeVec)));
    float angleOfIncidenceRadians = abs(acos(dot(normalizedPosVec, normalizedNormal)));
    if (angleToConeRadians < SensorConeHalfAngleRadians)
    {
        // Check the angle of incidence against both front and back facing polygons
        if ((angleOfIncidenceRadians < AngleOfIncidenceThresholdRadians) || 
            (angleOfIncidenceRadians > (PI-AngleOfIncidenceThresholdRadians)))
        {
            outColor = (distance(Pos, eyePos)*invDepthRange).xxxx;
            outColor.a = 1.0;
            float depth = outColor.b*MaximumRangeMM;
            if (depth < MinimumRangeMM)
            {
                outColor.rgb = 0.0;
            }
        }
        ...
        {
            // Culled due to incidence angle
            outColor.rgb = 1.0;
        }
    }
    else
    {
        // Culled due to sonar cone angle
        outColor.rgb = 1.0;
    }
    return outColor;
}
