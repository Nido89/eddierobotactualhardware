...
namespace Microsoft.Robotics.Services.SimpleDashboard
{
    ...
    /// </summary>
    [DisplayName("(User) Simple Dashboard")]
    ...
    [DssServiceDescription("http://msdn.microsoft.com/library/cc998494.aspx")]
    class SimpleDashboardService : DsspServiceBase
    {
        ...
        [ServiceState]
        [InitialStatePartner(Optional = true, ServiceUri = ServicePaths.Store + "/SimpleDashboard.config.xml")]
        SimpleDashboardState _state;
        ...
        /// </summary>
        public SimpleDashboardService(DsspServiceCreationPort creationPort) :
                base(creationPort)
        {
        }
        ...
        protected override void Start()
        {
            ...
            if (_state == null)
            {
                _state = new SimpleDashboardState { TranslateScaleFactor = 1, RotateScaleFactor = .25 };
            }
        ...
        {
            return new DriveControl(_eventsPort, _state.TranslateScaleFactor, _state.RotateScaleFactor);
        }
        ...
        IEnumerator<ITask> OnMoveHandler(OnMove onMove)
        {
            if (onMove.DriveControl == _driveControl && _drivePort != null)
            {
                ...
                request.LeftWheelPower = (double)onMove.Left * MOTOR_POWER_SCALE_FACTOR * _state.TranslateScaleFactor;
                request.RightWheelPower = (double)onMove.Right * MOTOR_POWER_SCALE_FACTOR * _state.TranslateScaleFactor;
        ...
        IEnumerator<ITask> OnConnectSickLRFHandler(OnConnectSickLRF onConnectSickLRF)
        {
            ...
            // shutdown the previous subscription to the LRF
        ...
        {
            var driveControlPort = _driveControl;
            if (driveControlPort != null)
            {
                WinFormsServicePort.FormInvoke(
                    delegate()
                    {
                        driveControlPort.ReplaceLaserData(replace.Body);
                    }
                );
            }
