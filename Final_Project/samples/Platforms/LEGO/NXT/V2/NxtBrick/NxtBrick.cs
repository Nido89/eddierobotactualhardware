                ...
                case LegoNxtPort.AnySensorPort:
                    ...
                    else if (reservation.Body.Connection.Port == LegoNxtPort.AnySensorPort)
                    {
                        if (reservation.Body.DeviceType == LegoDeviceType.DigitalSensor)
                        {
                            ...
                            if (!_state.Runtime.Connected)
                            {
                                // The hardware is not connected.  Can't check for I2C sensor!
                                done = true;
                                //// To find the I2C sensor, we need to talk to the brick.
                                //// Wait (up to 25 seconds) until the brick is connected before proceeding.
                                //int waitCount = 0;
                                //while (!_state.Runtime.Connected && waitCount++ < (25 * 4))
                                //    yield return Arbiter.Receive<DateTime>(false, TimeoutPort(250), EmptyHandler<DateTime>);
                            }
                ...
                yield return Arbiter.Choice(_brickPort.SendCommand(setInputMode),
                    delegate(LegoResponse ok)
                    {
                        if (!ok.Success)
                            ...
                            LogInfo(LogGroups.Console, string.Format("{0} set to I2C mode.", sensorPort));
